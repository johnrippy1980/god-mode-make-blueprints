...
function getDemoRecommendations(query, location) {
    const baseData = [
        {
...
async function scrapeWithFirecrawl(url, apiKey, log) {
    try {
...
function parseBusinessListings(content, query, location) {
    const businesses = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!line || line.length < 3)
            continue;
        if (/^(All|Posts|Businesses|Pages|For Sale|Search|Home|Nextdoor)/i.test(line))
            continue;
...
function buildNextdoorUrl(query, location, scrapeType) {
...
apify_1.Actor.main(async () => {
    ...
    const { scrapeType = 'businesses', location = 'Sausalito, CA', searchQuery = 'Plumber', searchUrl, category = 'all', maxResults = 50, includeComments = false, includeVotes = true, minRecommendations = 0, dateRange = 'all', proxyConfiguration, nextdoorEmail, nextdoorPassword, nextdoorCookies, useFirecrawl = false, firecrawlApiKey, demoMode = isEmptyInput, webhookUrl, } = input;
    ...
    if (demoMode) {
        ...
        if (scrapeType === 'recommendations' || scrapeType === 'businesses') {
            ...
            if (minRecommendations > 0) {
                results = results.filter(r => r.recommendationCount >= minRecommendations);
            }
        }
        else {
        ...
        // Push to dataset
        await apify_1.Actor.pushData(results);
    ...
    if (useFirecrawl && firecrawlApiKey) {
        ...
        const markdown = await scrapeWithFirecrawl(targetUrl, firecrawlApiKey, { info: console.log, warning: console.warn });
        if (markdown) {
    ...
    if (results.length === 0) {
        ...
        try {
            ...
            const camoufoxOptions = await (0, camoufox_js_1.launchOptions)({
                headless: true,
            });
            let proxyConfig;
            if (proxyConfiguration?.useApifyProxy) {
                proxyConfig = await apify_1.Actor.createProxyConfiguration({
    ...
    if (filteredResults.length > 0) {
        await apify_1.Actor.pushData(filteredResults);
        console.log(`\nSaved ${filteredResults.length} businesses to dataset`);
        // Charge for items scraped
        try {
            await apify_1.Actor.charge({ eventName: 'business_scraped', count: filteredResults.length });
    ...
    if (webhookUrl && filteredResults.length > 0) {
        try {
            ...
            console.log('Results delivered to webhook');
            try {
                await apify_1.Actor.charge({ eventName: 'webhook_delivered', count: 1 });
            }
            catch (e) {
                // Pay-per-event not available
            }
