...
interface BusinessListing {
    ...
    externalRatings: Array<{ source: string; rating: string }>;
    hours: string | null;
    address: string | null;
    phone: string | null;
    website: string | null;
...
async function deliverToWebhook(url: string, data: unknown): Promise<boolean> {
    try {
...
const {
    searchQueries = ['pizza austin tx', 'coffee shop san francisco'],
    maxResults = 20,
    demoMode = false,
...
if (demoMode) {
    ...
    const crawler = new PlaywrightCrawler({
        ...
        async requestHandler({ page, request }) {
            const searchQuery = request.userData.searchQuery as string;
            const isDetailPage = request.userData.isDetailPage as boolean;
            ...
            if (isDetailPage) {
                ...
                const existingIndex = results.findIndex(
                    r => r.name === request.userData.businessName && r.searchQuery === searchQuery
                );
            ...
            const listings = await page.evaluate((maxRes: number) => {
